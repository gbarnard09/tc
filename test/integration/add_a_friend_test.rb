...
class AddAFriendTest < ActionDispatch::IntegrationTest
 	...
 	test "that adding a friend works" do 
 		sign_in_as users(:ashley), "testing"

 		get "/user_friendships/new?friend_id=#{users(:freebird).profile_name}"
 		assert_response :success

 		assert_difference "UserFriendship.count" do
 			post "/user_friendships", user_friendship: { friend_id: users(:freebird).profile_name }
